...
allowed_filename = string.ascii_lowercase + string.ascii_uppercase + string.digits + "!#$%&'()-@^_`{}~+,.;=[]" + ''.join(
...
class PE:
    ...
    def parse_sections(self, offset):
        ...
        self.sections.sort(key=lambda x: x.VirtualAddress)
    ...
    def get_string_from_data(self, offset, data):
        ...
        return data[0:data.find(0, offset)].decode('ascii')
                ...
                for entry in self.FileInfo:
                    ...
                    if hasattr(entry, 'StringTable'):
                        ...
                        for st_entry in entry.StringTable:
                            ...
                            for str_entry in list(st_entry.entries.items()):
                                stringtable_dict[convert_to_printable(str_entry[0])] = convert_to_printable(
                                    str_entry[1])
                        ...
                        for var_entry in entry.Var:
                            ...
                            if hasattr(var_entry, 'entry'):
                                ...
                                var_dict[convert_to_printable(list(var_entry.entry.keys())[0])] = \
                                    list(var_entry.entry.values())[0]
